...
- 6列アイテムピッカー: `.jres` アイコン付き enum を12個ずつのページに分け、6列2段で表示するブロック
...
候補をたくさん用意して、`imagedropdown` の `fieldOptions.columns=6` を指定すると、6列に並ぶアイテム候補を作れます。ただし Minecraft Education の MakeCode では、候補が下に伸びてもドロップダウン内スクロールにならず、画面下で切れてしまうことがあります。

そのため、拡張側で確実に扱うなら、候補を12個ずつくらいのページに分けます。6列2段なら、Minecraft Education の画面でも切れにくくなります。
...
//% blockId=my_pick_paged_item_1
//% block="6列アイテムピッカー 1-12 で $item を選ぶ"
//% item.fieldEditor="imagedropdown"
//% item.fieldOptions.columns=6
//% item.fieldOptions.width=420
export function pickPagedItem1(item: PagedItem1): void {
}
```

これは検索付きの標準 `Item` ピッカーではなく、自作候補を固定の6列グリッドに整列させるための方法です。たくさんの候補を1つの `imagedropdown` に詰め込むと切れることがあるので、ページ別ブロックに分けるほうが安定します。
