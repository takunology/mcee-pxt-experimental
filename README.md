...
- 自作アドオンアイテム風ピッカー: `.jres` のロッド系アイコン付き enum で、アドオン側の item ID を選ぶブロック
...
MakeCode 側では、enum と `.jres` を使って候補を作り、選んだ値を `mcee:emerald_rod` のような item ID 文字列へ変換します。

```ts
enum AddonItem {
    //% block="エメラルドロッド"
    //% jres=AddonItemIcon.emeraldRod
    EmeraldRod,
    //% block="ルビーロッド"
    //% jres=AddonItemIcon.rubyRod
    RubyRod
}
...
export function addonItemId(item: AddonItem): string {
    if (item == AddonItem.RubyRod) return "mcee:ruby_rod";
...
enum ScrollItem {
    ...
    Item01 = 1,
    //% block="02 サファイア"
    //% jres=AddonItemIcon.sapphireRod
    Item02 = 2,
    // ...
    //% block="36 鋼エメラルド"
    //% jres=AddonItemIcon.steelEmeraldRod
    Item36 = 36
}
...
このリポジトリには、比較用に `画像サイズ比較` ブロックも入れています。同じエメラルドロッドの 16px / 32px / 64px 画像を `imagedropdown` に並べているので、Minecraft Education の MakeCode 上でどこまで見た目が変わるかを確認できます。

```ts
enum ImageSizeIcon {
    //% block="16px"
    //% jres=AddonItemIcon.rod16
    Rod16 = 16,
    //% block="32px"
    //% jres=AddonItemIcon.rod32
    Rod32 = 32,
    //% block="64px"
    //% jres=AddonItemIcon.rod64
    Rod64 = 64
}
