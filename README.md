...
- 自作アドオンアイテム風ピッカー: `.jres` アイコン付き enum で、アドオン側の item ID を選ぶブロック
...
- 上にくっつかないイベント型にしたい場合は `topblock=true` を付けます。
...
上に何もくっつかない、最初だけのような形です。関数の引数に `handler: () => void` を取り、`topblock=true` と `handlerStatement=true` を付けます。
...
MakeCode 側では、enum と `.jres` を使って候補を作り、選んだ値を `mcee:ruby` のような item ID 文字列へ変換します。

```ts
enum AddonItem {
    //% block="ルビー"
    //% jres=AddonItemIcon.ruby
    Ruby,
    //% block="まほうのカギ"
    //% jres=AddonItemIcon.key
    MagicKey
}
...
export function addonItemId(item: AddonItem): string {
    if (item == AddonItem.MagicKey) return "mcee:magic_key";
    return "mcee:ruby";
}
```
